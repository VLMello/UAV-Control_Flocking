...
HM_EPISODES = 2000000
MOVE_PENALTY = 1
ENEMY_PENALTY = 30 # Bater no lider
CLOSE_ENEMY_PENALTY = 5 # Degrau perto do lider
CLOSE_PENALTY_LINEAR = 100 # Slope perto do lider
FAR_PENALTY = 5 # Degrau longe do lider
...
SAFE_DISTANCE_LOW = 6 #4
SAFE_DISTANCE_HIGH = 9 #7
...
print(f"Starting SARSA training for {HM_EPISODES} episodes at {start_time_text}")
...
for episode in range(HM_EPISODES):
    ...
    for i in range(iter_per_episode):
        ...
        # Choose action (epsilon-greedy) according to current state (obs)
        ...
        # --- SARSA: choose next_action from new_obs using the same policy (epsilon-greedy) ---
        if terminal:
            ...
            if np.random.random() > epsilon:
                ...
                if player.roll == 30:
                    next_action = np.random.randint(0, CHOICES-1)
                elif player.roll == -30:
                    next_action = np.random.randint(1, CHOICES)
                else:
                    next_action = np.random.randint(0, CHOICES)
            next_q = q_table[new_obs][next_action]

        # Update current Q using SARSA target (on-policy)
        current_q = q_table[obs][action]
        if reward == SAFE_PLACE_REWARD and not terminal:
            ...
            sarsa_target = reward + DISCOUNT * next_q
            new_q = (1 - LEARNING_RATE) * current_q + LEARNING_RATE * sarsa_target
