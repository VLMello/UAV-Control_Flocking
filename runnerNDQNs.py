...
DQN_MODEL_FILES = [
    "dqn_planes_1772477044.pth", # 0 - With leader command
    "dqn_planes_1772506136.pth",  # 1 - No leader command
    "dqn_planes_1772677237.pth", # 0 - With leader command
    "dqn_planes_1772757948.pth"  # 1 - No leader command
    # Example: "dqn_planes_1768713482.pth",
    # Add your model paths here. If empty, script will try to duplicate DQN_MODEL_FILE below.
]

DQN_MODEL_NAMES = [
    "DQN Com Speed=1 e 90 Headings e sem Z6", # 0 - No leader command
    "DQN Com Speed=1 e 90 Headings e com Z6", # 0 - With leader command
    "DQN Com 64 neuronios", # 0 - No leader command
    "DQN Com 256 neuronios" # 0 - With leader command
    #"DQN Sem Z6"  # 1 - No leader command
    # Example: "dqn_planes_1768713482.pth",
    # Add your model paths here. If empty, script will try to duplicate DQN_MODEL_FILE below.
]
...
for idx in range(N_DQNS):
    ...
    if TORCH_AVAILABLE and model_path is not None and os.path.exists(model_path):
        try:
            m = DQN(hidden=DQN_MODEL_HIDDEN[idx]).to(device)
...
for idx in range(N_DQNS):
    print(f"{DQN_MODEL_NAMES[idx]} hits: {hitsD[idx]} (model_loaded={dqn_model_loaded_flags[idx]})")
...
for idx in range(N_DQNS):
    ...
    plt.plot(distsDavg_list[idx], color=clr, alpha=0.6, label=str(DQN_MODEL_NAMES[idx]))
