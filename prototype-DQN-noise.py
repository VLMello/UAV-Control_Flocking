...
SIZE = 200
SHOW_SIZE = 400
HM_EPISODES = 10000      # smaller by default; increase if you want longer training
...
epsilon = 0.9
EPS_DECAY = 0.998        # decay per episode
EPS_MIN = 0.02
SHOW_EVERY = 500
SHOW_ANY = False                                                           

MAX_NOISE_STEP = 0.2 #0.2
MAX_NOISE_HEADING = 0.4 #0.4
MAX_NOISE_ROLL = 0.5 #1
...
class Plane:
    ...
    def step(self, phi=False, follower=False):
        noiseRoll = np.random.normal(-MAX_NOISE_ROLL, MAX_NOISE_ROLL)
        if phi is not False:
            self.roll = max(min(self.roll + phi + noiseRoll, 30), -30)

        sr = self.get_discrete_roll()
        noiseHead = np.random.normal(-MAX_NOISE_HEADING, MAX_NOISE_HEADING)
        if sr == 4:
            self.heading = (self.heading + 2 + noiseHead) % HEADING_MAX
        if sr == 3:
            self.heading = (self.heading + 1 + noiseHead) % HEADING_MAX 
        elif sr == 1:
            self.heading = (self.heading - 1 + noiseHead) % HEADING_MAX
        elif sr == 0:
            self.heading = (self.heading - 2 + noiseHead) % HEADING_MAX
        ...
        if follower:
            ...
            noiseStep = np.random.normal(-MAX_NOISE_STEP, MAX_NOISE_STEP)
            self.x += self.speed * np.cos(np.deg2rad(self.heading * 360 / HEADING_MAX)) + noiseStep
            noiseStep = np.random.normal(-MAX_NOISE_STEP, MAX_NOISE_STEP)
            self.y += self.speed * np.sin(np.deg2rad(self.heading * 360 / HEADING_MAX)) + noiseStep
    ...
    def get_discrete_heading(self):
        return int(self.heading % HEADING_MAX)
...
def train():
    ...
    for episode in range(HM_EPISODES):
        ...
        for i in range(iter_per_episode):
            ...
            Z1_Z2 = get_tuple(enemy.x, enemy.y, player.x, player.y, enemy.get_discrete_heading() * 360 / HEADING_MAX)
            Z3 = (enemy.get_discrete_heading() - player.get_discrete_heading()) % HEADING_MAX
            ...
            # next observation
            Z1_Z2_n = get_tuple(enemy.x, enemy.y, player.x, player.y, enemy.get_discrete_heading() * 360 / HEADING_MAX)
            Z3_n = (enemy.get_discrete_heading() - player.get_discrete_heading()) % HEADING_MAX
