...
class Q_Learning:
    ...
    class Plane:
        ...
        def draw(self, outer, color):
            forward_point = (self.y * self.ZOOM + 30 * np.sin(np.deg2rad(self.heading*360/self.HEADING_MAX-0)), self.x * outer.ZOOM + 30 * np.cos(np.deg2rad(self.heading*360/outer.HEADING_MAX-0)))
            right_point = (self.y * self.ZOOM + 10 * np.sin(np.deg2rad(self.heading*360/self.HEADING_MAX+45)), self.x * outer.ZOOM + 10 * np.cos(np.deg2rad(self.heading*360/outer.HEADING_MAX+45)))
            left_point = (self.y * self.ZOOM + 10 * np.sin(np.deg2rad(self.heading*360/self.HEADING_MAX-45)), self.x * outer.ZOOM + 10* np.cos(np.deg2rad(self.heading*360/outer.HEADING_MAX-45)))
            ...
            ImageDraw.Draw(outer.img).circle([int(np.round(self.y, decimals=0)*self.ZOOM),int(np.round(self.x, decimals=0)*outer.ZOOM)], 5, fill=color, outline=None, width=0)
    ...
    def train(self):
        ...
        if self.start_q_table is None:
            ...
            print(f"QL - Initializing Q-table at {temp}")
        ...
        print(f"QL - Starting training for {self.HM_EPISODES} episodes at {start_time_text}")
        ...
        for episode in range(self.HM_EPISODES):
            ...
            hit = 0
            #test = Plane(int(self.SIZE/2), int(self.SIZE/2))
            if episode % self.SHOW_EVERY == 0:
                print(f"QL - on #{episode}, self.epsilon is {self.epsilon}")
                print(f"QL - {self.SHOW_EVERY} ep mean: {np.mean(episode_rewards[-self.SHOW_EVERY:])}")
                show = True
            else:
                show = False
            ...
            for i in range(self.iter_per_episode):
                ...
                if reward == self.SAFE_PLACE_REWARD:
                    ...
                    #new_q_speed = (1 - self.LEARNING_RATE) * current_q_speed + self.LEARNING_RATE * (reward + self.DISCOUNT * max_future_q_speed)
                ...
                if show:
                    ...
                    img = img.resize((self.SHOW_SIZE, self.SHOW_SIZE+100), resample=Image.NEAREST)  # resizing so we can see our agent in all its glory.
                    ...
                    enemy.draw(color=self.colors[self.ENEMY_N], outer=self)
                    player.draw(color=self.colors[self.PLAYER_N], outer=self)

                    ImageDraw.Draw(img).rectangle([(0, self.SHOW_SIZE), (self.SHOW_SIZE, self.SHOW_SIZE+100)], fill=(255, 255, 255))  # draws a black rectangle around the image
                    ...
                    ImageDraw.Draw(img).text((10, self.SHOW_SIZE+50), f"Player - X: {player.x:.2f}, Y: {player.y:.2f}, H: {player.heading}, R: {player.roll} , S: {player.speed}", fill=(self.colors[self.PLAYER_N]))
                    ImageDraw.Draw(img).text((10, self.SHOW_SIZE+70), f"Enemy - X: {enemy.x:.2f}, Y: {enemy.y:.2f}, H: {enemy.heading}, R: {enemy.roll} , S: {enemy.speed}", fill=(self.colors[self.ENEMY_N]))
                    #ImageDraw.Draw(img).text((10, self.SHOW_SIZE+90), f"Test - X: {test.x:.2f}, Y: {test.y:.2f}, H: {test.heading}, HC: {test.heading*360/self.HEADING_MAX}, sin: {np.sin(np.deg2rad(test.heading*360/self.HEADING_MAX)):.2f}, cos: {np.cos(np.deg2rad(test.heading*360/self.HEADING_MAX)):.2f}, R: {test.roll} , S: {test.speed}", fill=(0, 0, 0))
                    ...
                    if reward == self.SAFE_PLACE_REWARD:  # crummy code to hang at the end if we reach abrupt end for good reasons or not.
        ...
        print("QL - Finished. Q-table saved.")
